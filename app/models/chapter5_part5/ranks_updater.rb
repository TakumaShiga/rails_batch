module Chapter5Part5
  class RanksUpdater
    ...
    def create_ranks
      User.includes(:user_scores).find_in_batches(batch_size: 30) do |users|
        Rank.import users.select { |user| user.total_score.nonzero? }.map { |user| { user_id: user.id, score: user.total_score } }
      end 
